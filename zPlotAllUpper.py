...
plotFile = "zProfileAllHydrs.pdf"

# hydrLevels = [9]
hydrLevels = [9, 12, 15]
zMax = [71.5842764713, 74.1077055351, 78.7626683686]
...
for ihydr, hydrLevel in enumerate(hydrLevels):
    
    # fname = f'hydr_{str(hydrLevel)}_{atPairs}_avg_RDF.csv'
    # fnames = ['densAll.txt', 'densSulfurAll.txt']
    
    # for fname in fnames:
    fname = f'densAll{str(hydrLevel)}.txt'
    print(fname)
    df = pd.read_csv(fname, skiprows=4, index_col=False,  names=['i', 'fraction','number','density'], delim_whitespace=True)
    #,sep='\s+')
    
    print(df)
    print(df.columns)
    print(df.iloc[2])
    print(df.reindex(index=df.index[::-1]))
    ...
    ax1.set_ylabel('Total Mass Density ($g/cm^3$)')
    # ax1.plot(df['fraction']*zMax[ihydr], df['density'], label=f'$\lambda$={str(hydrLevel)}')
    ax1.set_xlabel('Z (Ang)')
    # ax1.plot(df['fraction']*zMax[ihydr], df['density'], label=f'$\lambda$={str(hydrLevel)}')
    ax1.plot((df.index.values)*0.01*zMax[ihydr], df['density'], label=f'$\lambda$={str(hydrLevel)}')
    # ax1.set_xlim(0, 0.2)
